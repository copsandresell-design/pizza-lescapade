...
export default function AdminOrdersPage() {
  ...
  const activeCount = orders.filter(
    (o) => o.statut !== 'cancelled' && o.statut !== 'ready'
  ).length
  ...
  return (
    <div className="min-h-screen" style={{ backgroundColor: '#1a0e06', color: '#fdf6ec' }}>
      ...
      >
        <div>
          <span
            style={{ fontFamily: 'var(--font-dancing), cursive', fontSize: '1.4rem', color: '#d4a843' }}
          >
            L&apos;Escapade · Admin
          </span>
          {activeCount > 0 && (
        ...
        <button
          onClick={load}
          className="p-2 rounded-full hover:bg-[#4a3020] transition-colors"
        >
          <RefreshCw size={16} style={{ color: '#d4a843' }} />
        </button>
      </header>
      ...
      <div
        ...
        style={{ borderBottom: '1px solid #4a3020' }}
      >
        {FILTERS.map((f) => (
          <button
            key={f.key}
            ...
            style={
              filter === f.key
                ...
                : { backgroundColor: '#3a2010', color: '#9a7c4e' }
            }
          >
            {f.label}
            {f.key !== 'all' && (
      ...
      <main className="max-w-5xl mx-auto px-4 py-6">
        {loading ? (
          <div className="text-center py-12" style={{ color: '#6b5040' }}>
            Chargement…
          </div>
        ) : visible.length === 0 ? (
            ...
            {visible.map((order) => (
              ...
              >
                ...
                <div
                  ...
                  style={{ backgroundColor: '#3a2010', borderBottom: '1px solid #4a3020' }}
                >
                  <div>
                    <span className="font-bold text-lg" style={{ color: '#d4a843' }}>
                      #{order.numero}
                    </span>
                    <span className="ml-2 text-xs" style={{ color: '#9a7c4e' }}>
                      {formatDate(order.createdAt)}
                    </span>
                  </div>
                  <Badge
                    bg={ORDER_STATUS_COLORS[order.statut]}
                    color="#fff"
                  >
                ...
                >
                  ...
                  {order.heureRetrait && (
                    <div
                      ...
                      style={{ backgroundColor: '#4a3020', color: '#d4a843' }}
                    >
                ...
                {order.note && (
                  <div
                    ...
                    style={{ backgroundColor: '#3a2010', color: '#9a7c4e' }}
                  >
                    Note : {order.note}
                  </div>
                )}
                ...
                <div
                  className="flex items-center justify-between px-4 py-3"
                  style={{ borderTop: '1px solid #4a3020' }}
                >
